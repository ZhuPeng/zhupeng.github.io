...
们使用的搜索语料文档是英文 Wikipedia 的部分文档，可以通过网站 dumps.wikimedia.org 下载。目前解压后的文档大小是 913MB，里面包含了超过 60 万的 XML 文件。示例如下：
...
现在我们已经将所有的文档加载到内容，那就可以开始尝试用最简单的方法一次去搜索包含 cat 的文档了。我们通过一个循环判断每一篇文档是否包含 cat：
...
任何英文文档中都包含了一些通用的词，比如 a、I、the、be。而这些词另外一个称呼是停用词。我们会将这次删除不进行索引，因为任何的文档中都包含这些词，搜索这些词并没有任何意义。
...
实现一个词干提取器并没有那么容易，而这并不是本片博文的重点，所以我们使用了一个现成的开源模块。
...
当我们利用倒排索引，搜索算法的时间复杂度是和搜索词的数量相当的，这意味着算法时间复杂度是 O(1)，并不会随着文档数量的增加而增加。
...
搜索结果是我们所期望的。同时，我们也是第一次听说 catopuma（金猫）， 以下图片就是。
...
我们仅仅是实现了一个简单的全文搜索引擎，尽管它很简单，但是依然基于此建立更多复杂的项目。

同时我们也没有过多的去介绍提高性能，以及使搜索引擎对用户使用更加的友好。以下是一些后续优化的方向仅供参考：
